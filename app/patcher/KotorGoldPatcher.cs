...
namespace KotorUniversalUI
{
    ...
    internal sealed class GoldPatch
    {
        ...
        internal const string PatchVersion = "1.1.0-gold-D8F0EEBF-ini";
        ...
        internal byte[] Apply(byte[] source)
        {
    ...
    internal sealed class ResolutionChoice
    {
        ...
        internal ResolutionChoice(int width, int height)
        {
            Width = width;
            Height = height;
            displayName = width.ToString(CultureInfo.InvariantCulture) + " × " +
    ...
    {
        private const string ResourceName = "KotorUniversalUI.override3440x1440";
        ...
        internal static OverrideEditState Install(string executablePath, Action<string> report)
        {
            return Install(executablePath, report, null);
        }

        internal static OverrideEditState Install(string executablePath, Action<string> report, Action<int, string> progress)
        {
            ...
            if (resource == null)
                throw new InvalidDataException("The bundled interface files are missing from this patcher.");
            ...
            try
            {
                ...
                using (ZipArchive archive = new ZipArchive(resource, ZipArchiveMode.Read, false))
                {
                    ...
                    SafeProgress(progress, 18, "Installing interface files…");
                    ...
                    foreach (ZipArchiveEntry entry in archive.Entries)
                    {
                        ...
                        if (existingInstallation)
                        {
                            if (!known.TryGetValue(relative, out record))
                                throw new InvalidDataException("The existing interface backup does not match this patcher. Restore it before patching again.");
                        }
                        else
                        {
                        ...
                        try
                        {
                            ...
                            if (String.IsNullOrEmpty(record.InstalledHash))
                                ...
                                throw new InvalidDataException("The bundled interface files do not match the existing backup record.");
                            ...
                            int percent = 18 + (int)Math.Min(76L,
                                completedBytes * 76L / Math.Max(1L, totalBytes));
                            SafeProgress(progress, percent, "Installing interface files…");
                        }
                        finally
                        {
    ...
    internal static class PatchOperations
    {
        ...
        internal static string Describe(string targetPath)
        {
            ...
            try
            {
                ...
                if (info.Length == GoldPatch.TargetLength && hash == GoldPatch.TargetHash)
            ...
            Action<int, string> progress)
        {
            ...
            if (width != IniOperations.DefaultWidth || height != IniOperations.DefaultHeight)
                throw new ArgumentOutOfRangeException("resolution", "This build currently supports only 3440 × 1440.");

            string currentHash = GoldPatch.HashFile(targetPath);
            SafeProgress(progress, 5, "Checking game files…");
            if (currentHash == GoldPatch.TargetHash)
            {
                ...
                try
                {
                    ...
                    existingOverrideState = OverrideOperations.Install(targetPath, report, progress);
                    SafeProgress(progress, 98, "Saving patch information…");
                    WriteManifest(targetPath, BackupPath(targetPath), false, width, height);
            ...
            try
            {
                ...
                byte[] target = patch.Apply(source);
                WriteVerifiedFile(temporaryPath, target, GoldPatch.TargetHash);
                File.Replace(temporaryPath, targetPath, null, true);
                installed = true;

                if (GoldPatch.HashFile(targetPath) != GoldPatch.TargetHash)
                ...
                overrideState = OverrideOperations.Install(targetPath, report, progress);
                SafeProgress(progress, 98, "Saving patch information…");
                WriteManifest(targetPath, backupPath, false, width, height);
                SafeProgress(progress, 100, "Patch complete");
                SafeReport(report, "KOTOR is ready to play at " +
                    width.ToString(CultureInfo.InvariantCulture) + " × " +
                    height.ToString(CultureInfo.InvariantCulture) + ".");
        ...
        internal static void ApplyToNewFile(string sourcePath, string outputPath)
        {
            ...
            GoldPatch patch = GoldPatch.Load();
            byte[] target = patch.Apply(File.ReadAllBytes(sourcePath));
            WriteVerifiedFile(outputPath, target, GoldPatch.TargetHash);
        }
        ...
        internal static void Restore(string targetPath, Action<string> report, Action<int, string> progress)
        {
            ...
            if (currentHash == GoldPatch.SourceHash)
            {
                ...
                WriteManifest(targetPath, BackupPath(targetPath), true, 0, 0);
            ...
            try
            {
                ...
                IniOperations.Restore(targetPath, report);
                SafeProgress(progress, 98, "Saving restore information…");
                WriteManifest(targetPath, backupPath, true, 0, 0);
                SafeProgress(progress, 100, "Restore complete");
                SafeReport(report, "The original game files and settings have been restored.");
            }
            finally
        ...
        private static void WriteManifest(string targetPath, string backupPath, bool restored, int width, int height)
        {
            ...
            string json = "{\r\n" +
                ...
                "  \"goldSha256\": \"" + GoldPatch.TargetHash + "\"\r\n" +
                "}\r\n";
    ...
    internal sealed class CompatibilityDialog : Form
    {
            ...
            Action<IWin32Window> browseForFile)
        {
            ...
            body.Text = "KOTOR needs the editable swkotor.exe from Deadly Stream before the ultrawide update can be installed.\r\n\r\n" +
    ...
    internal sealed class MainForm : Form
    {
        ...
        internal MainForm()
        {
            ...
            explanation.Text = "Set up KOTOR for ultrawide play at the selected resolution.\r\n" +
                "The patcher updates the game, display settings, and matching interface files.";
            ...
            resolutionBox.SetBounds(154, 181, 200, 28);
            resolutionBox.Items.Add(new ResolutionChoice(3440, 1440));
        ...
        private void RefreshStatus()
        {
            ...
            if (state == ExecutableState.SupportedClean)
            {
                ...
                detail = iniExists ?
                    "Everything is ready. Choose PATCH GAME to install the ultrawide update. " + IniOperations.Describe(target) :
            ...
            else if (state == ExecutableState.Gold)
            {
                ...
                detail = iniExists ?
                    "The executable is already patched. You may safely re-apply the selected INI resolution." :
    ...
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            try
            {
                ...
                {
                    PatchOperations.ApplyToNewFile(args[1], args[2]);
                    return 0;
                }
