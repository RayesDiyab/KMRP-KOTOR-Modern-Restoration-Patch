...
| [binary-inventory.md](binary-inventory.md) | **Every one of the 893 bytes that differ between the clean executable and gold**, grouped into 77 runs, each tied to the document that explains it. Generated by `tools/build_binary_inventory.py`, which exits non-zero if any run has no write-up — the one check here that is organised by bytes rather than by subject, and so the only one that can notice a patch site nobody documented. |
