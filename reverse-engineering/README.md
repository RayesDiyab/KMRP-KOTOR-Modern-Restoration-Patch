...
| [area-map-surface.md](area-map-surface.md) | **The map surface itself:** the canvas, the marker overlay, and the `LBL_Map` control that shows them. Why the area map has an unfogged 242 px strip on its right at 3440x1440, the three scaling schemes (vanilla, k1hrm/hires_patcher, KMRP) compared across 49 resolutions, the canvas placement rule, and the two candidate fixes with their open risks. Lab record in `experiments/area-map-surface/`. |
| [map-markers.md](map-markers.md) | The icons drawn *on* the map — notes, party, player arrow. All fourteen size and centring sites, the two draw paths a map note has, the scale rule and the alternative that was rejected after play-test. |
